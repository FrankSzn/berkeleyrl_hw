...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, q_values):
        ...
        if self.nn_baseline:
            b_n_unnormalized = self.actor.run_baseline_prediction(obs)#TODO
            b_n = np.mean(q_values) + np.multiply(b_n_unnormalized, np.std(q_values))
            adv_n = q_values - b_n #TODO
    ...
    def _discounted_return(self, rewards):
        ...
        discounted_rewards = np.multiply(rewards, discounts)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            indices = list(range(start_time_index,len(rewards))) #TODO

            # 2) create a list where the entry at each index (t') is gamma^(t'-t)
            discounts = np.power(self.gamma, np.subtract(indices, start_time_index))#TODO
            ...
            discounted_rtg = np.multiply(discounts, rewards[start_time_index:])#TODO
